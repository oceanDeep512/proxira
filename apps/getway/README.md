...
- **自定义请求头** - 每个转发地址可配置固定请求头，并按名称前缀匹配替换或忽略（可覆盖多个头）
- **新请求提示** - 没停在最新一条时浮出「N 条新请求」，一点跳到最新；停在最新时静默跟随不打扰
- **请求详情概览** - 按「时间 → 请求 → 响应 → 来源」铺开全部字段，时间精确到毫秒
- **请求重放** - 改完参数直接重发上游，与原响应逐行差异对比
- **正文查看** - JSON 树（行号 / 逐层折叠 / 大数组分页 / 节点复制）、源码视图、表格、SSE 逐帧、多格式高亮
- **正文内查找** - 用浏览器自带的查找（⌘F / Ctrl+F），面板不再自带搜索框
