...
/**
 * 请求头配置编辑器（固定头 + 按名称前缀匹配的改写规则）。
 *
 * 这是原先独立弹窗 `RequestHeadersModal` 的内容部分抽出后的形态：设置面板把入口
 * 收进「请求头」标签后，弹窗壳就没用了 —— 直接内嵌在这里，切标签即见到已保存的配置。
 *
 * 数据模型仍是**转发地址级**的（每个地址一套），所以顶部会标明当前作用于哪个地址；
 * 升级为可复用预设分组后的形态见后续轮次。
 *
 * 校验以服务端为准（非法名称、被代理接管的头都会 400 并带上具体原因），
 * 这里只挡掉整行空白这种「显然后台也存不下」的输入。
 */
export const HeaderRulesEditor = ({
  targetId,
  targetName,
  customHeaders,
  headerRules,
  ...
  const [headers, setHeaders] = useState<HeaderDraft[]>(() => customHeaders.map(toHeaderDraft));
  const [rules, setRules] = useState<RuleDraft[]>(() => headerRules.map(toRuleDraft));
  const [saving, setSaving] = useState(false);

  // 只在挂载或切换转发地址时灌入草稿：跟随 props 会让打字过程中被 SSE 推来的
  // 新配置覆盖掉。切到别的标签再回来会重新挂载，也就重新取到最新已保存的值。
  useEffect(() => {
  ...
  if (!targetId) {
    return (
      <EmptyState
        title="还没有转发地址"
        hint="请求头配置挂在转发地址上，先在左侧新增一个转发地址。"
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <p className="m-0 text-[12px] leading-relaxed text-fg-soft">
        当前作用于转发地址
        <strong className="mx-1 font-medium text-fg">{targetName || "未命名"}</strong>
        的出站请求；切到其他转发地址会换成那一套配置。
      </p>
      ...
      <section className="flex flex-col gap-2.5">
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <Button
            variant="secondary"
            size="sm"
            onClick={() => setHeaders((current) => [...current, { id: draftId(), name: "", value: "" }])}
          >
            ...
            {rules.map((rule) => (
              ...
              >
                ...
                <Input
                  ...
                  onChange={(event) => patchRule(rule.id, { namePrefix: event.target.value })}
