...
>
> 服务默认只监听 `127.0.0.1`，其他电脑连不上；要让局域网内的设备访问用 `--host lan`（详见下方「局域网共享」）。
>
...
启动时加 `--host lan`（等价于 `--host 0.0.0.0`，即监听所有网卡）：
...
| `--host <address>` | 监听地址，`lan` 或 `0.0.0.0` = 局域网可访问 | `127.0.0.1` |
