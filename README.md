...
- **局域网共享**：启动时打印可直接发给同事的 `Network` 地址。默认只监听 `127.0.0.1`，此时不展示（同事连不上）；
  需要共享用 `--host 0.0.0.0`，并注意只在可信网络里开。
- **窄屏自适应**（≤960px）：标题与转发地址同行；历史请求折叠为可点菜单（整行热区，折叠态右侧显示当前选中请求）；
  内容区跟随父盒子缩放，被压缩时在容器内部横向滚动。

> [!NOTE]
> 上面三项的详细说明见 [apps/getway/README.md](apps/getway/README.md)（即 npm 包首页文档）。
