...
- 设置面板：顶栏齿轮集中管理——数据存储文件夹（含一键在文件管理器中打开）、主题切换、清空所有数据；请求头配置直接内嵌在此管理，拦截规则入口也在这里。对话框固定高度，切换标签不跳高，内容超出滚动。
...
配置挂在**转发地址**上，切到别的地址就换一套，只作用于该地址的出站请求。设置面板的「请求头」标签内直接列出当前转发地址已保存的固定头与匹配规则，可增删改并保存。
