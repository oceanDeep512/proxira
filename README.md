...
- 新请求提示：没停在最新一条时浮出「N 条新请求」，点一下跳到最新；停在最新时静默跟随、不打扰。
...
来了新请求时，面板底部会浮出一条「N 条新请求 · GET /path · 200 · 32ms · 查看最新」的胶囊：

- **停在最新一条时不提示也不打断**，而是静默把详情推进到新请求（你本来就在看最新的，跟着走才是预期行为）。
- **你点开了某条旧请求之后再来的请求**才会累加计数并浮出提示，详情不会被硬切走 —— 正读着一条请求被抢走焦点是最烦人的行为。计数是准确的：跟随期间的请求不会「静默落后」成少算的一条。
- 点胶囊跳到最新一条并清空计数；右侧的 `✕` 只忽略提示、不跳转。
- 提示固定在视口底部居中，不占布局高度，也不会和右下角的消息 toast 抢位置；窄屏只保留「N 条新请求 + 查看最新」，路径与耗时让位。
